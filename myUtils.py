...
def find_ID(Pathstr):
    return Pathstr.split("/")[3] + "/" + Pathstr.split("/")[4]
...
def oneEpoch_iterator(list_tuples, person_to_images_map, batch_size=16, resize_picture=()):
    ppl = list(person_to_images_map.keys())
    batches = chunker(list_tuples, batch_size)
    for id, bat in enumerate(batches):
        labels = [1] * len(bat)
        # create a batch where annotation 0 means no relation and 1 means has, the data is (p1,p2).
        # Therefore the data-annotation(element of a batch) pair have such form (p1,p2)-0,(p3,p4)-1 .....
        while len(bat) < batch_size:
...
def unzip(list_tuple):
    return [list_tuple[i][0] for i in range(0, len(list_tuple))], \
           [list_tuple[i][1] for i in range(0, len(list_tuple))]
